...
const server = new index_js_1.Server({ name: "nexus-mcp-server", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    if (name === "plan_intent") {
        const { prompt, steps } = args;
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            // Insert Intent (Initialize as PLANNING)
            const intentResult = await client.query("INSERT INTO intents (raw_prompt, status) VALUES ($1, 'PLANNING') RETURNING id", [prompt]);
            const intentId = intentResult.rows[0].id;
            // Insert Execution Steps dynamically maintaining step_order
            for (let i = 0; i < steps.length; i++) {
                const step = steps[i];
                await client.query("INSERT INTO execution_steps (intent_id, step_order, target_name, payload, status) VALUES ($1, $2, $3, $4, 'pending')", [intentId, i + 1, step.target_name, step.payload]);
            }
            // Mark intent as COMPLETED now that plan generation is finalized
            await client.query("UPDATE intents SET status = 'COMPLETED', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [intentId]);
            // Log successful generation to audit_logs
            await client.query("INSERT INTO audit_logs (event_type, details) VALUES ($1, $2)", ["plan_generation_success", `Successfully mapped intent ID ${intentId} into ${steps.length} ordered execution steps.`]);
            ...
            await pool.query("INSERT INTO audit_logs (event_type, details) VALUES ($1, $2)", ["plan_generation_failure", `Failed to map prompt plan. Error: ${error.message}`]);
    ...
    if (name === "update_step_status") {
        const { step_id, status, error_message } = args;
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            // Update step status and error_message if provided
            await client.query("UPDATE execution_steps SET status = $1, error_message = $2 WHERE id = $3", [status, error_message || null, step_id]);
            // Write changes directly into your audit logs table
            await client.query("INSERT INTO audit_logs (event_type, details) VALUES ($1, $2)", [`step_${status}`, `Execution step ID ${step_id} transitioned to state: ${status}.${error_message ? ' Error: ' + error_message : ''}`]);
            await client.query("COMMIT");
